...
class Experiment:
    ...
    def update_batch_info(self, trainvalid_dict, dataset_type, output, target, weights, batch_idx, epoch_idx, num_of_batches):
        
        batch_loss = self.criteria['objective_criteria'](output, target, weights)
        trainvalid_dict[dataset_type][self.args['objective_criteria']]['epochs_sum'][epoch_idx] += batch_loss
        if (batch_idx == num_of_batches - 1):
               trainvalid_dict[dataset_type][self.args['objective_criteria']]['epochs_mean'][epoch_idx] = trainvalid_dict[dataset_type][self.criteria['name']]['epochs_sum'][epoch_idx] / num_of_batches      
        for criteria in self.args['additional_criteria']:
            batch_loss = self.criteria(output,target,weights)
            trainvalid_dict[dataset_type][criteria.name]['epochs_sum'][epoch_idx] += batch_loss
            if (batch_idx == num_of_batches - 1):
                trainvalid_dict[dataset_type][criteria.name]['epochs_mean'][epoch_idx] = trainvalid_dict[dataset_type][self.criteria['name']]['epochs_sum'][epoch_idx] / num_of_batches
