...
def divide_set_in_season(dataset, num_cat, hirst_data_path):
    ...
    #num_cat = 4
    dataset["Category"] = pd.qcut(dataset["AMBR"], num_cat, labels=range(num_cat), retbins=True)[0]
    train_set = []
    valid_set = []
    num_of_train = 0
    num_of_valid = 0
...
def divide_set_out_season(dataset, num_cat):
    #num_cat = 4
    dataset["Category"] = pd.qcut(dataset["Total"], num_cat, labels=range(num_cat), retbins=True)[0]
    train_set = []
    valid_set = []
    num_of_train = 0
    num_of_valid = 0
